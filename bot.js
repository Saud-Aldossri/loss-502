...
var bestsupports = ['303197992809267202','273380454151749633','','','','','','','','',''];// ايديات الي يقدرو يضيفو
client.on('message',async message => {
...
client.on('message' , message => {
  ...
  if(message.content.startsWith(prefix + "addS")) {
    ...
    if(message.content.startsWith(prefix + "remS")) {
      let args = message.content.split(' ').slice(1).join(' ');
      if(!isNaN(args)) return;
      if(!serverslist.has(args)) return message.replay('**هذا السيرفر ليس موجود بـ القائمه**')
